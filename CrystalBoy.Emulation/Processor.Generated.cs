...
namespace CrystalBoy.Emulation
{
	...
	{
		internal bool Emulate(bool shouldFinishFrame)
		{
#if WITH_DEBUGGING
			return Emulate((shouldFinishFrame ? 0 : 1) | (bus.BreakpointCount > 0 ? 2 : 0));
#else
			return Emulate(shouldFinishFrame ? 0 : 1);
#endif
		}

		/// <summary>Emulates instructions until the completion conditions are met.</summary>
		/// <remarks>
		/// The completion flags indicate whether a condition other than a frame completion can stop the emulation:
		/// The value 1 indicates completion as soon as possible.
		/// The value 2 indicates that at least one breakpoint is defined.
		/// </remarks>
		/// <param name="completionFlags">Flags used to determine compeltion of the method.</param>
		/// <returns>true if the emulation completed as result of a frame completion; otherwise false.</returns>
		private bool Emulate(int completionFlags)
		{
			...
			try
			{
				while (true)
				{
					...
					switch (opcode)
					{
						...
						case 0x76: /* HALT */
							...
							{
								bus.AddCycles(4); // Add the 4 cycles now… In order to adjust interrupts
				...
				OnInstructionEmulated: ;
#if WITH_DEBUGGING
					if (!bus.AddCycles(cycleCount))
					{
						return completionFlags == 0 || (completionFlags & 2) != 0 && !bus.IsBreakpoint(pc);
					}
					else if (completionFlags != 0 && ((completionFlags & 1) != 0 || bus.IsBreakpoint(pc)))
					{
						return false;
					}
#else
					if (completionFlags != 0 || bus.AddCycles(cycleCount)) return completionFlags == 0;
#endif
				}
			}
			finally
